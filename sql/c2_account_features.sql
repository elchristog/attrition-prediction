...
-- ══════════════════════════════════════════════════════════════
-- CTE 2b: PROGRAM_ID per account (from NAM_ACCOUNT_ATTRITION)
-- ══════════════════════════════════════════════════════════════
account_program AS (
    SELECT
...
fleet_snapshot_mthly AS (
    SELECT
        ...
        MAX(f.outstanding_card_count)         AS outstanding_cardcount_mth
...
sr_monthly_casetypes AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN LOWER(c.case_primary_reason) ILIKE '%pump%'                      THEN c.case_id END) AS case_pump_issues_count,
        COUNT(DISTINCT CASE WHEN LOWER(c.case_primary_reason) != 'termination - non vas qualifying' THEN c.case_id END) AS case_total_excl_term_count,
        ...
        ) AND LOWER(c.case_primary_reason) NOT ILIKE '%pump%' THEN case_id END)                   AS case_other_count
...
account_month_spine AS (
    SELECT
        ...
        ap.program_id
    FROM base_data b
    LEFT JOIN account_program ap ON ap.account_id = b.account_id
...
monthly_combined AS (
    SELECT
        ...
        COALESCE(sct.case_pump_issues_count, 0)             AS case_pump_issues_count,
        COALESCE(sct.case_total_excl_term_count, 0)         AS case_total_excl_term_count,
...
SELECT
    ...
    fsm.outstanding_cardcount_mth,

    -- ── SMALL BUSINESS FLAG ──
    -- Based on outstanding cards in the current month snapshot
    CASE
        WHEN COALESCE(fsm.outstanding_cardcount_mth, 0) < 10 THEN 'Yes'
        ELSE 'No'
    END AS is_small_biz,
    ...
    mc.case_pump_issues_count,
    mc.case_total_excl_term_count,
    mc.case_other_count,
    mc.cr_limit_mth,
